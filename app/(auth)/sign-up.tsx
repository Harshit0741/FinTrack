...
export default function SignUpScreen() {
  ...
  const passwordRules = [
    { id: 'len', text: "At least 15 characters", fulfilled: passwordValue.length >= 15 },
    { id: 'up', text: "One uppercase letter", fulfilled: /[A-Z]/.test(passwordValue) },
    { id: 'low', text: "One lowercase letter", fulfilled: /[a-z]/.test(passwordValue) },
    { id: 'num', text: "One number", fulfilled: /[0-9]/.test(passwordValue) },
    { id: 'spec', text: "One special character", fulfilled: /[^A-Za-z0-9]/.test(passwordValue) },
  ];
    ...
    if (signUp.status === "complete") {
      await signUp.finalize({
        navigate: ({ session, decorateUrl }) => {
          ...
          const url = decorateUrl("/(root)/onboarding");
      ...
      <View className="flex-1 justify-center px-6 -mt-16">
          ...
          render={({ field: { value, onChange } }) => {
            return (
              <View className="relative mb-2">
                <TextInput
                  className="border border-[#E8E6DF] bg-white rounded-xl px-4 py-3 text-[#1A1D26] pr-12"
        ...
        <View className="mb-4 ml-1">
          {passwordRules.map((rule) => (
            <RequirementItem key={rule.id} text={rule.text} fulfilled={rule.fulfilled} />
          ))}
        </View>
...
const RequirementItem = ({ text, fulfilled }: { text: string; fulfilled: boolean }) => {
  const animatedIconStyle = useAnimatedStyle(() => {
    return {
      transform: [{ scale: withTiming(fulfilled ? 1 : 0.8) }],
      opacity: withTiming(fulfilled ? 1 : 0.5),
    };
  });

  const color = fulfilled ? "#3DDC84" : "#8A8D96";

  return (
    <View className="flex-row items-center gap-2 mb-1.5">
      <Animated.View style={animatedIconStyle}>
        <Feather name={fulfilled ? "check-circle" : "circle"} size={14} color={color} />
      </Animated.View>
